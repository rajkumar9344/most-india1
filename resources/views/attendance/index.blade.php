...
@section('content')
    ...
    <div class="container">
        ...
        <div id="leaveModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="leaveModalLabel"
            aria-hidden="true">
            <!-- Modal content -->
            <button type="button" class="close leave-modal-close" data-dismiss="modal" aria-label="Close">
        ...
        <script>
            ...
            $(document).ready(function() {
                ...
                $('#leaveModal').on('hidden.bs.modal', function() {
                    ...
                    const leaveStatus = $(`.status-dropdown[data-employee-id="${employeeId}"]`).val();


                    // If leave has not been submitted, set status back to 'present'
                    if (leaveStatus !== 'leave') {
                        $(`.status-dropdown[data-employee-id="${employeeId}"]`).val('present');
                    }
                });
                        ...
                        $.ajax({
                            ...
                            success: function() {
                                console.log('Leave detail.', employeeId, leaveDate, leaveType,
                                    leaveReason);
                                // Change the employee's status to 'leave' after saving the leave details
                                $(`.status-dropdown[data-employee-id="${employeeId}"]`).val(
                                    'leave');
